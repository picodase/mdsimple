...
gmx trjconv -f md_0_1.trr -s md_0_1.tpr -o md_0_1_10ps.pdb -dump 10
# Choose option 1 Protein

# Potential energy analysis
# gmx g_energy -f md_0_1.edr -o md_0_1_PE.xvg

# Low-pass filter for making smooth movies
# gmx g_filter -f md_0_1.trr -s md_0_1.tpr -ol md_0_1_resampled.xtc -all

# Center in box and fix boundary conditions
gmx trjconv -f md_0_1.trr -s md_0_1.tpr -o md_0_1_nojump.xtc -center -pbc nojump

# Generate a PDB file for visualizing the nojump trajectory
gmx trjconv -f md_0_1_nojump.xtc -s md_0_1.tpr -o md_0_1_nojump.pd
