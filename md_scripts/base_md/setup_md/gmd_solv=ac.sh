...
# Obtain your PDB file of the object you wish to use in the simulation. Ensure you either parameterize it for an .itp file and PDB then convert it to .gro (editconf), or convert it to gro + top (pdb2gmx) if it has residues which meet their forcefields.
#gmx pdb2gmx -f obj.pdb -o obj_proc.gro # outputs "topol.top"
#gmx editconf -f obj.pdb -o obj_proc.gro 
#add "#include "obj.itp" and "obj.top" " if present 

# Place your new parameterized object in a box for the simulation!
gmx editconf -f ${GROMACS_PDB}_processed.gro -o ${GROMACS_PDB}_newbox.gro -c -d 1.0 -bt cubic

# If you don't have a box of solvent yet, you can use gmx solvate or packmol, but be sure the density seems appropriate!
# If using acetone for a solvent, ensure you have the box as a .gro file in the current working directory. I named mine CAS_box.gro, but you can pick another name. Download the .itp file for acetone and #include it 
# Solvate the box
gmx solvate -cp ${GROMACS_PDB}_newbox.gro -cs CAS_box.gro -o ${GROMACS_PDB}_solv.gro -p topol.top
...
gmx grompp -f params/ions.mdp -c ${GROMACS_PDB}_solv.gro -p topol.top -o ions.tpr
...
gmx grompp -f params/em.mdp -c ${GROMACS_PDB}_solv_ions.gro -p topol.top -o em.tpr
