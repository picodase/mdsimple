...
# Gromacs parameters
LIGAND=$1				# Take in first cmdlnarg
PDB_REMOVE="HOH"
...
GROMACS_BOXSIZE="2.5"				# Def boxsize
GROMACS_BOXCENTER="1.25"			# Def boxcenter

# Setup GROMACS Job. Probably not necessary to edit past this point.
#if [ -z "$JLN_SIM_PATH/$LIGAND" ]; then
...
#grep -v $PDB_REMOVE ${LIGAND}.pdb > ${LIGAND}_clean.pdb	# water molecules
#grep -v OKH ${LIGAND}.pdb > ${LIGAND}_clean.pdb	# ligand molecules
#grep -v HOH ${LIGAND}_clean.pdb > ${LIGAND}_nohho.pdb	# save a model w/o H2O for later
...
#pdb2gmx -f ${LIGAND}_clean.pdb -o ${LIGAND}_processed.gro -water ${GROMACS_WATERMODEL}	# Process the file after removing water
gmx pdb2gmx -f ${LIGAND}.pdb -o ${LIGAND}_processed.gro -water ${GROMACS_WATERMODEL}		# Process file with crystal waters

# Edit the box by changing its dimensions
gmx editconf -f ${LIGAND}_processed.gro -o ${LIGAND}_newbox.gro -c -d 1.0 -bt cubic

# Solvate the box
gmx solvate -cp ${LIGAND}_newbox.gro -cs spc216.gro -o ${LIGAND}_solv.gro -p topol.top
...
gmx grompp -f ions.mdp -c ${LIGAND}_solv.gro -p topol.top -o ions.tpr

# Generate ions inside the box
gmx genion -s ions.tpr -o ${LIGAND}_solv_ions.gro -p topol.top -pname NA -nname CL -neutral

# Download an input parameter file
wget http://www.mdtutorials.com/gmx/lysozyme/Files/minim.mdp

# Assemble the binary input 
gmx grompp -f minim.mdp -c ${LIGAND}_solv_ions.gro -p topol.top -o em.tpr
